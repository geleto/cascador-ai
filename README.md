...
#### How to Create It
*   **Default (Plain Text)**: The `prompt` is a static string with no processing. You can also provide a `ModelMessage[]` array in the `prompt` property to define a multi-message prompt.
    ```typescript
...
*   **With a Template (`.withTemplate`)**: The `prompt` is a Cascada template string.
    ...
    ```
*   **With a Script (`.withScript`)**: The `prompt` is a Cascada script. The script can return a `string` (which becomes the prompt) or a `ModelMessage[]` array (which gets appended to the message history).
    ```typescript
    const scriptGenerator = create.TextGenerator.withScript({ model: openai('gpt-4o'), script: ':data @data = "Summarize the key points from: " + article' });
    ```
...
*   **With a Template (`.withTemplate`)**:
    ...
    ```
*   **With a Script (`.withScript`)**: The script can return a prompt string or a full `ModelMessage[]` array.
    ```typescript
    const scriptStreamer = create.TextStreamer.withScript({ model: openai('gpt-4o'), script: ':data @data = "Write a story based on this premise: " + premise' });
    ```
...
*   **With a Template (`.withTemplate`)**: The `prompt` is a Cascada template string.
    ...
    ```
*   **With a Script (`.withScript`)**: The `prompt` is generated by a Cascada script's output.
    ```typescript
    const scriptObjGenerator = create.ObjectGenerator.withScript({ model: openai('gpt-4o'), schema: z.object({ ... }), script: ':data @data = "Extract key entities from: " + text' });
    ```
*   **Loading from a resource (`.loads...`)**: Load the `prompt` from a file. It can be treated as plain text (`.loadsText`), a template (`.loadsTemplate`), or a script (`.loadsScript`).
    ```typescript
...
*   **With a Script (`.withScript`)**: The `prompt` is generated by a Cascada script's output.
    ```typescript
    const scriptObjStreamer = create.ObjectStreamer.withScript({ model: openai('gpt-4o'), schema: z.object({ ... }), script: ':data @data = "Generate characters based on: " + theme' });
    ```
*   **Loading from a resource (`.loads...`)**: Load the `prompt` from a file. It can be treated as plain text (`.loadsText`), a template (`.loadsTemplate`), or a script (`.loadsScript`).
    ```typescript
...
For text-only renderers (those not created with `.withTemplate` or `.withScript`), you can provide an array of messages directly in the `prompt` property at configuration time. This is useful for defining few-shot examples or complex initial prompts.
```typescript
