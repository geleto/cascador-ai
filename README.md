...
*   **With a Template (`.withTemplate`)**: The `prompt` is a Cascada template string.
    ...
    ```
*   **With a Script (`.withScript`)**: The `prompt` is generated by a Cascada script's output. The script can return a string (which becomes the prompt) or an array of `ModelMessage` objects (which becomes the entire message history).
    ```typescript
